"""
Functionality for reading PALSAR ALOS 1 data into a SICD model.

Reference Document: PALSAR ALOS 1 Level 1.1 Product Format Description
    JAXA ERSDAC CEOS Format L1.1
    -  https://earth.esa.int/eogateway/documents/20142/37627/ALOS-PALSAR-Product-Format-Description-Level-1-1.5.pdf
    
Reference Code    : PALSAR ALOS 2

Attention: Because Native Doppler,the iso-Doppler geometry is different,
    i.e., distortion occurs in the ground geometry in the azimuth direction.
"""

__classification__ = "UNCLASSIFIED"
__author__ = "Syusuke Yasui"
...
def _make_float(bytes_in, float_half=True):
    """
...
class _LED_Radiometric(_BaseElements):
    ...
    def __init__(self, fi):
        ...
        self.num_pts = int(fi.read(4))  # type: int
        self.cal_factor = numpy.float16(fi.read(16))  # type: float
        self.tx_distortion = numpy.zeros((2, 2), dtype='complex64')  # type: numpy.ndarray
        self.rcv_distortion = numpy.zeros((2, 2), dtype='complex64')  # type: numpy.ndarray
        for i in range(2):
            for j in range(2):
                self.tx_distortion[i, j] = complex(real=_make_float(fi.read(16), True), imag=_make_float(fi.read(16), True))
        for i in range(2):
            for j in range(2):
                self.rcv_distortion[i, j] = complex(real=_make_float(fi.read(16), True), imag=_make_float(fi.read(16), True))
        fi.seek(start_loc + self.rec_length, os.SEEK_SET)  # skip remaining reserved
...
class PALSARDetails(object):
    ...
    def _get_sicd(self,
        ...
        def get_collection_info() -> CollectionInfoType:
            if self._led_element.data.scene_id.startswith('ALPSRP'):
                collector_name = 'ALPSRP'
            else:
                collector_name = None
        ...
        def get_radiometric() -> RadiometricType:
            sigma_zero = 10**(0.1*(led_element.radiometric.cal_factor))
...
def is_a(file_name: str) -> Optional[PALSARReader]:
    """
    Tests whether a given file_name corresponds to a PALSAR ALOS file. Returns a reader instance, if so.
    ...
    try:
        ...
        logger.info('File {} is determined to be a PALSAR ALOS file.'.format(file_name))
