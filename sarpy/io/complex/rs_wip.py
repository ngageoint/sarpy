...
def _validate_chipper_and_sicd(the_sicd, chipper, name):
    """
    ...
    if data_size[0] != rows or data_size[1] != cols:
        raise ValueError(
            'The {} chipper construction got incompatible sicd size ({}, {}) and '
            'chipper size {}'.format(name, rows, cols, data_size))
...
def _construct_tiff_chipper(the_sicd, the_file, symmetry):
    ...
    _validate_chipper_and_sicd(the_sicd, chipper, 'tiff')
    return chipper
...
def _construct_single_nitf_chipper(the_sicd, the_file, symmetry):
    ...
    _validate_chipper_and_sicd(the_sicd, chipper[0], 'Single NITF')
    return chipper[0]
...
def _construct_multiple_nitf_chippers(the_sicds, the_file, symmetry):
    ...
    for i, (the_sicd, chipper) in enumerate(zip(the_sicds, chippers)):
        _validate_chipper_and_sicd(the_sicd, chipper, 'NITF band {}'.format(i))
    return chippers
...
class RadarSatDetails(object):
    ...
    __slots__ = (
        ...
        '_polarizations', '_x_spline', '_y_spline', '_z_spline',
    ...
    def _build_location_spline(self):
        ...
        for i, entry in enumerate(tie_points):
            ...
            if grid_col >= len(samples) or grid_row >= len(lines) or line != lines[grid_row] or sample != samples[grid_col]:
    ...
    def _get_regular_sicd(self):
        ...
        def get_radar_collection():
            ...
            radar_collection.RcvChannels = [
                ChanParametersType(TxRcvPolarization=entry, index=i+1) for i, entry in enumerate(tx_rcv_pols)]
            if len(tx_pols) == 1:
                radar_collection.TxPolarization = tx_pols[0]
            else:
                radar_collection.TxPolarization = 'SEQUENCE'
                radar_collection.TxSequence = [
                    TxStepType(TxPolarization=entry, index=i+1) for i, entry in enumerate(tx_pols)]
        ...
        def get_rma_adjust_grid():
            ...
            if collection_info.RadarMode.ModeType == 'SPOTLIGHT':
                ...
                raise ValueError('ScanSAR should be processed as Spotlight')
        ...
        def get_data_file_names():
            ...
            if self.generation == 'RS2':
                ...
                results = self._findall(data_file_tag)
                ...
                if len(results) == 1:
                    ...
                    for pol in self._polarizations:
                        fname = self._find(data_file_tag+'[@pole="{}"]'.format(pol)).text
                        if fname is None:
                            raise ValueError('Got unexpected image file structure.')
    ...
    def _get_scansar_sicd(self, beam, burst):
        ...
        def get_image_and_geo_data():
            ...
            im_data = ImageDataType(
                NumRows=rows, NumCols=cols, FirstRow=0, FirstCol=0, PixelType=pixel_type,
                FullImage=(rows, cols), SCPPixel=(scp_rows, scp_cols))
            geo_data = GeoDataType(SCP=SCPType(ECF=scp_ecf))
            return im_data, geo_data, sample_offset
        ...
        def get_rma_adjust_grid():
            ...
            num_lines_processed = float(sar_processing_info.find('./numberOfLinesProcessed').text)
            ...
            line_spacing_zd = \
                get_seconds(zero_dop_last_line, zero_dop_first_line, precision='us')/num_lines_processed
            # NB: this will be negative for Ascending

            # zero doppler time of SCP relative to collect start -
            # NB: I've set collect start as the start of processing for this burst
            time_scp_zd = image_data.SCPPixel.Col*line_spacing_zd
        ...
        def get_radiometric():
            ...
            def perform_radiometric_fit(component_file):
                ...
                if numpy.all(comp_values == comp_values[0]):
                    ...
                    num_vs = int(comp_struct.find('./numberOfValues').text)
                    range_inds = pfv + numpy.arange(num_vs)*step
                    coords_rg = grid.Row.SS * (range_inds - (image_data.SCPPixel.Row + row_shift))
                    return numpy.reshape(polynomial.polyfit(coords_rg, comp_values, 3), (-1, 1))
        ...
        def get_data_file_names():
            ...
            if len(results) == 1:
                ...
                for pol in self._polarizations:
                    fname = self._find(data_file_tag+'[@pole="{}"]'.format(pol))
                    if fname is None:
                        raise ValueError('Got unexpected image file structure')
                    image_files.append(os.path.join(base_path, fname.text))
            return image_files
        ...
        # NB: I am neglecting that prfInformation is provided for multiple pols
