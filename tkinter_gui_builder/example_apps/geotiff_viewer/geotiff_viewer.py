...
class GeotiffViewer(AbstractWidgetPanel):
    ...
    def __init__(self, master):
        ...
        self.select_file(os.path.expanduser("~/GreenIsland.tif"))
    ...
    def callback_update_red_band(self, event):
        ...
        band_num = 0
        if red_band == "None":
            if band_num not in self.geotiff_image_panel.variables.canvas_image_object.drop_bands:
                self.geotiff_image_panel.variables.canvas_image_object.drop_bands.append(band_num)
        else:
            if band_num in self.geotiff_image_panel.variables.canvas_image_object.drop_bands:
                self.geotiff_image_panel.variables.canvas_image_object.drop_bands.remove(band_num)
            self.image_reader.display_bands[band_num] = int(red_band)
        self.geotiff_image_panel.update_current_image()
    ...
    def callback_update_green_band(self, event):
        ...
        if green_band == "None":
            if band_num not in self.geotiff_image_panel.variables.canvas_image_object.drop_bands:
                self.geotiff_image_panel.variables.canvas_image_object.drop_bands.append(1)
        else:
            if band_num in self.geotiff_image_panel.variables.canvas_image_object.drop_bands:
                self.geotiff_image_panel.variables.canvas_image_object.drop_bands.remove(1)
